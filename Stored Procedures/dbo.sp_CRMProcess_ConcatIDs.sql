...
WHERE TM.[GUID] = z.[GUID] AND tm.[SourceSystem] = 'Paciolan' --updateme
...
FROM (SELECT CustomID1,[GUID], MAX(CAST(Primary_Flag as int)) Primary_Flag 
		FROM stg.tbl_CRMProcess_NonWinners WHERE [SourceSystem] = 'Paciolan'group by CustomID1,[GUID]) acct --updateme
...
UPDATE a
SET SSB_CRMSYSTEM_SSID_TIX__c = ISNULL(LTRIM([b].[ConcatIDs1]),'')
, [AccountId] = ISNULL(LTRIM([b].[ConcatIDs1]),'')
, SSB_CRMSYSTEM_DimCustomerID__c = ISNULL(LTRIM([b].[DimCust_ConcatIDs]),'')
...
INNER JOIN stg.[tbl_CRMProcess_ConcatIDs] b ON a.[SSB_CRMSYSTEM_CONTACT_ID__c] = b.GUID
