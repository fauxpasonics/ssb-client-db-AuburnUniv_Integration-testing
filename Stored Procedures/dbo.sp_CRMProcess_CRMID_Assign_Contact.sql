...
--FROM dbo.contact c
--LEFT JOIN (SELECT ssb_crmsystem_contact_id FROM dbo.vwDimCustomer_ModAcctId  WITH (NOLOCK) WHERE SourceSystem = 'AuburnUniv PC_SFDC Account' AND IsDeleted = 0) ssb
...
WHERE b.SourceSystem = 'Auburn PC_SFDC Account' AND ISNULL(a.crm_id, '') != b.ssid --updateme
AND c.[crm_id] IS NULL 
...
--WHERE dc.SourceSystem = 'Auburn PC_SFDC Account' --updateme
