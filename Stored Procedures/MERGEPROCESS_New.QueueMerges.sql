...
--Exec  [MERGEPROCESS_New].[QueueMerges] 'auburnuniv'
	@Client VARCHAR(100) 
AS
--Declare @Client VARCHAR(100) ='Raiders'
DECLARE @Account VARCHAR(100) = (SELECT CASE WHEN @client = 'AuburnUniv' THEN 'Auburn PC_SFDC Account' ELSE CONCAT(@client,' PC_SFDC Account' ) END);
DECLARE @Contact VARCHAR(100) = (SELECT CASE WHEN @client = 'AuburnUniv' THEN 'Auburn PC_SFDC Contact' ELSE CONCAT(@client,' PC_SFDC Contact' ) END );
...
WHERE ((AutoMerge = 1 and NumRecords < 20) OR Approved = 1)
