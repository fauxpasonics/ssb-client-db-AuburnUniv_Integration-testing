...
--[MERGEPROCESS_New].[MergeDetect]  'auburnuniv'
	@Client VARCHAR(100) 
AS
Declare @Date Date = (select cast(getdate() as date));
DECLARE @Account varchar(100) = (Select CASE WHEN @client = 'AuburnUniv' THEN 'Auburn PC_SFDC Account' ELSE Concat(@client,' PC_SFDC Account' ) END); --updateme
...
ON dm.PK_MergeID = fm.FK_MergeID AND ISNULL(fm.complete,0) = 0
