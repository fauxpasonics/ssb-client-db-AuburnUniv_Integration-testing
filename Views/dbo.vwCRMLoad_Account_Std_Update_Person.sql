...
SELECT a.ssb_crmsystem_Contact_ID__c, a.FirstName, a.LastName,a.Suffix, a.BillingStreet, a.BillingCity, a.BillingState, a.BillingPostalCode, a.BillingCountry, a.Phone, a.Id, [LoadType], a.Name AS FullName__c
...
AND  (HASHBYTES('SHA2_256',ISNULL(LTRIM(RTRIM(CAST( a.FirstName AS VARCHAR(MAX)))),'') )  <> HASHBYTES('SHA2_256',ISNULL(LTRIM(RTRIM(CAST( c.FirstName AS VARCHAR(MAX)))),'')) 
	...
	OR HASHBYTES('SHA2_256',ISNULL(LTRIM(RTRIM(CAST( a.name AS VARCHAR(MAX)))),'') )  <> HASHBYTES('SHA2_256',ISNULL(LTRIM(RTRIM(CAST( c.fullname__c AS VARCHAR(MAX)))),'')) 
...
--updateme once suffix__c is in place
GO
